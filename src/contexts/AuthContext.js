...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated' && session?.user) {
      ...
      // Show notification for new social logins
      // Check if this is coming from a callback (new login) vs page refresh
      const isCallback = typeof window !== 'undefined' && 
        (window.location.href.includes('callback') || 
         sessionStorage.getItem('social-login-pending') === 'true')
      
      if (!wasAuthenticated && session.user.name && (isCallback || !state.user)) {
