...
export default function CosmeticsPage() {
  ...
  useEffect(() => {
    fetchCosmeticsProducts()
  }, [])

  const fetchCosmeticsProducts = async () => {
    try {
      const response = await fetch("/api/products?category=Cosmetics")
      if (response.ok) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1">
        {/* Hero Section */}
        <div className="py-20 text-center text-white" style={{ backgroundColor: "#DBCCB7", color: "#5A0117" }}>
          <div className="max-w-4xl mx-auto px-4">
            ...
            <p className="text-xl md:text-2xl opacity-90" style={{ fontFamily: "Montserrat, sans-serif" }}>
              Premium beauty products for your everyday glamour
            </p>
          </div>
        </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            {loading ? (
              ...
              <div className="text-center py-20">
                ...
                >
                  No Cosmetics Products Found
                </h3>
                <p className="text-lg" style={{ color: "#8C6141", fontFamily: "Montserrat, sans-serif" }}>
                  Check back soon for our latest cosmetics collection.
                </p>
              </div>
            )}
          </div>
