...
export default function LoginPage() {
  const [loginMethod, setLoginMethod] = useState("email") // email, otp, oauth
  ...
  const handleOTPResend = async () => {
    return await sendOTP(formData.email, "login")
  }
        ...
        <div className="max-w-md w-full">
          <div className="text-center mb-8">
            ...
            <p className="mt-2" style={{ fontFamily: "Montserrat, sans-serif", color: "#8C6141" }}>
              Sign in to your Kanvei account
            </p>
          </div>
          <div className="bg-white rounded-lg shadow-md p-8">
            <div className="flex mb-6 border-b">
              <button
                ...
                className={`flex-1 py-2 px-4 text-sm font-semibold border-b-2 transition-colors cursor-pointer ${
                  loginMethod === "email" ? "border-current" : "border-transparent"
                }`}
                style={{
                  color: loginMethod === "email" ? "#5A0117" : "#8C6141",
              ...
              <button
                ...
                className={`flex-1 py-2 px-4 text-sm font-semibold border-b-2 transition-colors cursor-pointer ${
                  loginMethod === "otp" ? "border-current" : "border-transparent"
                }`}
                style={{
                  color: loginMethod === "otp" ? "#5A0117" : "#8C6141",
                  fontFamily: "Montserrat, sans-serif",
                }}
              >
                Email OTP
            ...
            {loginMethod === "email" && (
              <form onSubmit={handleEmailLogin} className="space-y-6">
                ...
                <div>
                  ...
                  <input type="password" name="password" value={formData.password} onChange={handleChange} required className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-opacity-50" style={{ fontFamily: "Montserrat, sans-serif" }} placeholder="Enter your password" />
                </div>
                <div className="text-right text-sm mb-4">
                  <Link href="/forgot-password" className="font-semibold hover:opacity-80 transition-opacity" style={{ color: "#5A0117", fontFamily: "Montserrat, sans-serif" }}>
                    Forgot Password?
                  </Link>
                </div>
            ...
            <div className="mt-6 text-center">
              <p style={{ fontFamily: "Montserrat, sans-serif", color: "#8C6141" }}>
                ...
                <Link href="/register" className="font-semibold hover:opacity-80 transition-opacity" style={{ color: "#5A0117" }}>
                  Sign up
                </Link>
              </p>
            </div>
