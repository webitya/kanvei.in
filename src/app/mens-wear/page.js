...
export default function MensWearPage() {
  ...
  const fetchMensWearProducts = async () => {
    try {
      const response = await fetch("/api/products?category=Men's Wear")
      if (response.ok) {
        const data = await response.json()
        setProducts(data)
      }
    } catch (error) {
      console.error("Error fetching men's wear products:", error)
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="py-20 text-center text-white" style={{ backgroundColor: "#5A0117" }}>
          <div className="max-w-4xl mx-auto px-4">
            ...
            <p className="text-xl md:text-2xl opacity-90" style={{ fontFamily: "Montserrat, sans-serif" }}>
              Sophisticated fashion for the modern gentleman
            </p>
          </div>
        </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            {loading ? (
              ...
              <div className="text-center py-20">
                ...
                >
                  No Mens Wear Products Found
                </h3>
                <p className="text-lg" style={{ color: "#8C6141", fontFamily: "Montserrat, sans-serif" }}>
                  Check back soon for our latest mens fashion collection.
                </p>
              </div>
            )}
          </div>
