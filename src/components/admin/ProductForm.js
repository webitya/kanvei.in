...
export default function ProductForm({ product, onSubmit, onCancel }) {
  ...
  const [step, setStep] = useState(product?.category ? 2 : 1)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const optionsArray = (formData.options || []).map((opt) => ({
        size: opt.size || "",
        price: opt.price === "" ? "" : Number.parseFloat(opt.price),
        mrp: opt.mrp === "" ? "" : Number.parseFloat(opt.mrp),
        color: opt.color || "",
        stock: opt.stock === "" ? "" : Number.parseInt(opt.stock),
      ...
      const submitData = {
        ...
        price: formData.price === "" ? "" : Number.parseFloat(formData.price),
        mrp: formData.mrp === "" ? "" : Number.parseFloat(formData.mrp),
        weight: formData.weight === "" ? "" : Number.parseFloat(formData.weight),
        height: formData.height === "" ? "" : Number.parseFloat(formData.height),
        width: formData.width === "" ? "" : Number.parseFloat(formData.width),
        stock: formData.stock === "" ? "" : Number.parseInt(formData.stock),
      ...
      const data = await res.json()
      console.log('[ProductForm] Outgoing payload:', submitData)
      console.log('[ProductForm] API response:', data)
      if (data?.success) {
        alert('Product prepared and stored in DB collections successfully (attributes/images/options/views).')
